...
class Dispatcher(DiGraph):
    def __init__(self,*args,**kwargs):
        DiGraph.__init__(self,*args,**kwargs)
        self.counter=Counter()
        self.start = 'start'
        self.add_node(self.start,attr_dict={'type':'start'})
    ...
    def add_data(self,id=None,data=None,wait_inputs=False,**kwargs):
        ...
        if id is None:
            id = self.counter()
            while self.has_node(id):id = self.counter()
        self.add_node(id,attr_dict=attr_dict)
        return id

    def add_function(self,fun,outputs,inputs=OrderedDict(),input_domain=None,**kwargs):
        ...
        for u,attr in inputs.items():
            if not self.has_node(u): self.add_data(id=u)
            self.add_edge(u,id,attr_dict=attr)

        for v,attr in outputs.items():
            if not self.has_node(v): self.add_data(id=v)
            self.add_edge(id,v,attr_dict=attr)

    def get_data_sources(self):
        return [k for k,v in self.node.items() if (v.get('type',None)=='data') and (v.get('outputs',None) is not None)]

    def set_starting_node(self,sources=None):
        self.remove_node(self.start)

        self.add_node(self.start,attr_dict={'type':'start'})

        for v in (sources if sources else self.get_data_sources()):self.add_edge(self.start,v,{'weight':1})
    ...
    def populate_output(self,targets=None,cutoff=None):
        ...
        while fringe:
            ...
            for w,edgedata in self[v].items():
                vw_dist = dist[v] + edgedata.get('weight',1)

                node = self.node[w]; node_type, wait_inputs = (node.get('type',None), node.get('wait_inputs',False))

                if not start and (node_type in ('data','function')):
                    node['inputs'] = node.get('inputs',{}); output=self.node[v]['outputs']
                    node['inputs'][v] = output[w] if node_type == 'data' else output

                if (cutoff is not None and vw_dist>cutoff) or \
                   (wait_inputs and not set(self.predecessors(w)).issubset(dist)): continue
    ...
    def remove_cycles(self):
        ...
        active_nodes,edge_deleted=delete_cycles(network)

        network=network.subgraph(active_nodes)

        for e in edge_deleted:
            network.remove_edge(*e)
            if not network.out_degree(e[0]): network.remove_node(e[0])

        return network

    def plot(self):
        from networkx import spring_layout,draw_networkx_nodes,draw_networkx_edges,draw_networkx_labels,get_node_attributes
        pos=spring_layout(self)
        start,data,function=([],[],[])
        for k,type in ((k,v.get('type',None)) for k,v in self.node.items()):
            eval(type).append(k)
        labels = get_node_attributes(self,'outputs')
        labels = {k:'%s:%s'%(str(k),str(v)) for k,v in labels.items()}
        labels[self.start] = 'start'
        draw_networkx_nodes(self,pos,node_shape='^',nodelist = start,node_color='b')
        draw_networkx_nodes(self,pos,node_shape='o',nodelist = data,node_color='r')
        draw_networkx_nodes(self,pos,node_shape='s',nodelist=function,node_color='y')
        draw_networkx_labels(self,pos,labels=labels)
        draw_networkx_edges(self,pos,alpha=0.5)
        plt.axis('off')
...
def define_network():
    ...
    for i in range(n_data):
        ...
        v = i<2
        network.add_data(data=i+1 if v else None,wait_inputs=False if v else choice([True,False]))
    ...
    for i in range(n_function):
        ...
        network.add_function(fun(len(outputs)),outputs,
                             inputs = OrderedDict([(data.pop(),{'weight':1}) for v in range(choice([1,2]))]))
    '''

    network.add_function(fun(2),{v:{'weight':1} for v in [2,4]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [0,1]]))
    network.add_function(fun(1),{v:{'weight':1} for v in [3]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [1,2]]))
    network.add_function(fun(1),{v:{'weight':1} for v in [4]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [2,3]]))
    network.add_function(fun(1),{v:{'weight':1} for v in [5]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [4]]))
    network.add_function(fun(2),{v:{'weight':1} for v in [3,0]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [5]]))
    network.add_function(fun(2),{v:{'weight':1} for v in [0,1]},
                             inputs = OrderedDict([(v,{'weight':1}) for v in [2]]))
    return network
...
if __name__ == '__main__':
    ...
    for (k,v) in network_without_cycles.node.items():
        print(k,v)
